...
- 機能設計を始める前に、必ず `.claude/guides/ddd.md` を読むこと
- データモデルの設計では、エンティティ・値オブジェクト・集約を区別して定義する
- 用語は `docs/glossary.md`（ユビキタス言語定義）と統一する
- ビジネスルールはドメインモデルの責務として記述する（画面やサービスの処理として書かない）
